...
@cache.df_cache()
def chadwick_register(save: bool = True) -> pd.DataFrame: # MANUAL CHANGE: added save argument to control whether to save the register to cache after pulling from source. Default is True to maintain existing behavior.
    ...
    # MANUAL CHANGE: added print statement and breakpoint here to inspect the table after processing. Remove or comment out these lines in production.
    print(f"Finished gathering player lookup table with {len(table)} players.")
    print(table.head())  # MANUAL CHANGE: added print statement to inspect the table after processing. Remove or comment out this line in production.
    print(table.dtypes)  # MANUAL CHANGE: added breakpoint here to inspect the table before saving to cache. Remove or comment out this line in production.
